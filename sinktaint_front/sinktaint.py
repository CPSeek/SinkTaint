...
from front_analysise.untils.config import ANALYSIZER, B_FILTERS, F_FILTERS, API_SPLIT_MARCH, FROM_BIN_ADD, UPNP_ANALYSISE
...
def argsparse():
    ...
    parser = argparse.ArgumentParser(description="SinkTaint tool",
    ...
    group.add_argument("-b", "--bin", required=False, nargs="+", action="append", metavar="/var/ac18/bin/httpd", help="Input border bin")
...
def ghidra_analysise(args, border_bin: list):
...
def main():
    ...
    for binname, binpath in bin_list:
        ...
        if not os.path.isdir(bin_ghidra_project):
            os.makedirs(bin_ghidra_project)
        bin_ghidra_file = os.path.join(ghidra_result_output, binname, binname)
        analyze_bin(binpath, bin_ghidra_file)
        # try:
        #     analyze_bin(binpath, bin_ghidra_file)
        # except:
        #      continue

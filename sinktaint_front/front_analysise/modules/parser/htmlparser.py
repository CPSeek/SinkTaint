...
class HTMLParser(BaseParser):
    ...
    def get_keyword(self, html):
        ...
        pattern = re.compile(r'<INPUT name=(.*?)>', re.I)
        new_name_list = pattern.findall(html_content)
        temp_name_list = []
        for para in new_name_list:
            if ' ' in para:
                para = para.split(' ')[0]
            temp_name_list.append(para)
        pattern = re.compile(r'<input type=hidden name=(.*?)>', re.I)
        new_name_list = pattern.findall(html_content)
        for para in new_name_list:
            if ' ' in para:
                para = para.split(' ')[0]
            temp_name_list.append(para)
        new_name_list = temp_name_list
        results = set(name_list) | set(id_list) | set(new_name_list)

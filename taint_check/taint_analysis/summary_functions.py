...
def sprintf(_core, call_site_path, plt_path):
    ...
    for i in range(2, 2 + n_vargs):
        if i not in s_index:
            continue
        if i >= len(ordered_argument_regs[p.arch.name]):
            break
...
def memcpy_sized(_core, call_site_path, plt_path):
    ...
    if simplify_memcpy and plt_state_cp.solver.max(size) >= plt_state_cp.libc.max_memcpy_size:
...
def sizeof(_core, call_site_path, plt_path):
    ...
    # not tainted, but symbolic and huge
        ...
        for i in range(0, plt_state_cp.libc.max_str_len):
            cnt_i = _core.safe_load(plt_path_cp, n + i, 1)
            vals = plt_state_cp.solver.eval_upto(cnt_i, 2)
...
def _malloc(_core, _, plt_path):
    ...
    state = plt_path.active[0]
    # sim_size = getattr(state.regs, arg_reg_name(p, 0))
    sim_size = getattr(state.regs, ret_reg_name(p))

    if state.solver.symbolic(sim_size):
        size = state.solver.max_int(sim_size)
        if size > state.libc.max_variable_size:
            size = state.libc.max_variable_size
    else:
        size = state.solver.eval(sim_size)
    ...
    setattr(state.regs, ret_reg_name(p), addr)
    return sim_size
